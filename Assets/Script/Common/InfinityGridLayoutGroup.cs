 ...
 * Usage: ScrollView结构  ScrollRect/viewport/content/items
 * content 挂 Content Size Fitter, GridlayoutGroup, 本脚本, GridlayoutGroup设置Constraint Column
 * 所有节点pivot使用0,1
 ***************************/
...
public class InfinityGridLayoutGroup : MonoBehaviour 
{
        ...
        if (gridLayoutGroup.constraint == GridLayoutGroup.Constraint.FixedColumnCount)
        {
            ...
            if (offsetY > 0)
            {
                //向上拉，向下扩展;
                {
                    ...
                    //Debug.LogError(string.Format("childBottom :{0}, scrollRectUp,{1}", childBottom, scrollRectUp));
    ...
    public void SetAmount(int count, bool init = true)
    {
        amount = count;
